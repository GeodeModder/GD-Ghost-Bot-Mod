...
struct RuntimeGhost {
    ...
    bool disappearPinged = false; // debug-only: fires once when this ghost vanishes, showing exact timing
};
...
public:
    ...
    bool m_rewindPinged = false; // debug-only: fire the rewind popup once per session, not once per death
    ...
    void rewindToLastCheckpoint() {
        ...
        // Fires once per session only - this used to fire on every single
        // checkpoint death, and repeated deaths while grinding a hard section
        // could stack many undismissed popups, likely causing the save-dialog
        // button freeze reported after checkpoint-heavy runs.
        if (!m_rewindPinged) {
            m_rewindPinged = true;
            GhostDebug::popup("Ghost Debug",
                "Rewound from " + std::to_string(before) +
                " to " + std::to_string(m_recordingBuffer.size()) + " frames (further rewinds won't repeat this popup)");
        }
    }
    ...
    // Kept alive so any reset event (death, retry, entering practice mode)
    // can safely re-spawn sprites without needing that context passed in again.
    cocos2d::CCLayer* m_objectLayerRef = nullptr;

    // Renamed/reused as respawnAllSprites: GD's own resetLevel (and likely
    // togglePracticeMode) tears down and rebuilds the level's object layer
    // internally, which silently destroys our injected ghost sprites too.
    // We only used to spawn them once at init() - now we call this again
    // after every known reset event so the ghost is self-healing instead
    // of silently going stale (which looked like "disappears, no popup,
    // never comes back" and "never appears in practice mode at all").
    void respawnAllSprites(cocos2d::CCLayer* parentLayer) {
        if (parentLayer) m_objectLayerRef = parentLayer;
        if (!m_objectLayerRef) return;
        ...
        for (auto& ghost : m_loadedGhosts) {
            // Drop our reference without calling into it - if GD already
            // tore it down internally, the old pointer may be dangling and
            // unsafe to touch. We just forget it and make a fresh one.
            ...
            if (ghost.sprite) {
                ...
                ghost.disappearPinged = false;
                m_objectLayerRef->addChild(ghost.sprite);
            } else {
    ...
    void tickPlayback(float dt) {
        ...
        for (auto& ghost : m_loadedGhosts) {
            ...
            if (ghost.playbackElapsedTime > ghost.frames.back().time) {
                ...
                if (!ghost.disappearPinged) {
                    ghost.disappearPinged = true;
                    GhostDebug::popup("Ghost Debug",
                        ghost.routeName + " disappeared. Recorded duration: " +
                        std::to_string(ghost.frames.back().time) + "s (" +
                        std::to_string(ghost.frames.size()) + " frames). Playback reached: " +
                        std::to_string(ghost.playbackElapsedTime) + "s");
                }
            }
        }
    }
