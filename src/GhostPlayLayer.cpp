...
class $modify(GhostPlayLayer, PlayLayer) {
    ...
    // layer, potentially invalidating our old sprite pointer. Always pass
    // this->m_objectLayer fresh here (not a cached pointer) - caching caused
    // respawned sprites to attach to a stale, invisible layer instance.
    ...
    // Uses rewindToLastCheckpoint() instead of a separate full-wipe path -
    // that function already correctly clears everything when no checkpoints
    // exist, and using one consistent path avoids resetLevel unexpectedly
    // wiping recording progress if it fires as a side effect of something
    // else (like toggling practice mode), which was cutting recordings
    // short (500 frames instead of 2000+).
    void resetLevel() {
